...
def create_search_view(request: requests.Request, sqlite_db_handler: SqliteDBHandler = Depends(sqlite_db_handler)):
...
def get_models(manufacturer: str, sqlite_db_handler: SqliteDBHandler = Depends(sqlite_db_handler)):
    db = sqlite_db_handler.get_db_connection()
    models = db.query(CarModel.model).filter(CarModel.manufacturer == manufacturer).all()
