...
LOCAL_DB_CONTAINER = "postgres_db"
...
def start_local_db() -> None:
    logger.info("Starting %s container with exposed port 5432...", LOCAL_DB_CONTAINER)
...
"""
    ...
    try:
        subprocess.run(["docker", "compose", "up", "-d", LOCAL_DB_CONTAINER], check=True)
    finally:
        if os.path.exists(override_file):
            os.remove(override_file)

    _wait_for_healthy(LOCAL_DB_CONTAINER)


def start_or_reuse_test_db() -> None:
    if _container_health_status(TEST_DB_CONTAINER) == "healthy":
    ...
    _wait_for_healthy(TEST_DB_CONTAINER)
